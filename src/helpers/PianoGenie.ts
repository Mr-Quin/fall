 ...
 */
function sampleLogits(logits: tf.Tensor1D, temperature?: number, seed?: number) {
...
class PianoGenieBase {
    ...
    // Model state.
    private modelVars: { [varName: string]: tf.Tensor } | undefined
    private decLSTMCells: tf.LSTMCellFunc[] | undefined
    private decForgetBias: tf.Scalar | undefined
    // Execution state.

    // Execution state.
    private lastState: LSTMState | undefined
     ...
     * them (e.g. 60).
     */
    nextWithCustomSamplingFunction(button: number, sampleFunc: (logits: tf.Tensor1D) => tf.Scalar) {
    ...
    private evaluateModelAndSample(
        ...
        const [finalState, output]: [LSTMState, number] = tf.tidy(() => {
            ...
            const [c, h] = tf.multiRNNCell(
                this.decLSTMCells!,
                rnnInput as tf.Tensor<tf.Rank.R2>,
                initialState.c,
                initialState.h
            )
...
class PianoGenieAutoregressiveDeltaTime extends PianoGenieBase {
    ...
    private deltaTimeOverride: number | undefined
    ...
    protected getRnnInputFeats() {
        ...
        const feats: tf.Tensor1D = tf.tidy(() => {
            ...
            const lastOutputOh = tf.cast(
                tf.oneHot(lastOutputInc, NUM_PIANOKEYS + 1),
                'float32'
            ) as tf.Tensor1D
            ...
            const deltaTimeInt = tf.cast(tf.add(deltaTimeTrunc, 1e-4), 'int32') as tf.Tensor1D
...
class PianoGenieAutoregressiveDeltaTimeChord extends PianoGenieAutoregressiveDeltaTime {
    private chordRoot: PitchClass = 1
    private chordFamily: ChordFamily = 1
    ...
    protected getRnnInputFeats() {
        ...
        const feats: tf.Tensor1D = tf.tidy(() => {
            ...
            const chordRootTensorOh = tf.cast(
                tf.oneHot(chordRootTensorSubOne, 12),
                'float32'
            ) as tf.Tensor1D
...
class PianoGenieAutoregressiveDeltaTimeKeysig extends PianoGenieAutoregressiveDeltaTime {
    private keySignature: PitchClass = 1
    ...
    protected getRnnInputFeats() {
        ...
        const feats: tf.Tensor1D = tf.tidy(() => {
            ...
            const keySigTensorOh = tf.cast(
                tf.oneHot(keySigTensorSubOne, 12),
                'float32'
            ) as tf.Tensor1D
